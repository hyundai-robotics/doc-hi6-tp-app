# Table of contents

* [${cont_model} 로봇제어기 기능설명서 - Teach Pendant 앱(App)](README.md)
* [1. 개요](1-intro/README.md)

* [2. 설치](2-installation/README.md)
  * [2.1 준비](2-installation/1-preparation.md)
  * [2.2 설치](2-installation/2-install.md)
  
* [3. 설정과 실행](3-config-run/README.md)
  * [3.1 단축키(hotkey)와 실행방식](3-config-run/1-hotkey-startup.md)
  * [3.2 실행과 전환](3-config-run/2-run-switch.md)

* [4. S 릴레이에 의한 모니터링과 제어](4-sw-relay.md)
